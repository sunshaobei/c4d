...
object CreateResourceBloc {
     ...
     */
    const val MAX_API_FOR_MIX_RESOURCES = _root_ide_package_.android.os.Build.VERSION_CODES.O_MR1

    fun create(archiveFilePath: String, hostAppContext: _root_ide_package_.android.content.Context): _root_ide_package_.android.content.res.Resources {
        triggerWebViewHookResources(hostAppContext)

        val packageManager = hostAppContext.packageManager
        val applicationInfo = _root_ide_package_.android.content.pm.ApplicationInfo()
        ...
        if (_root_ide_package_.android.os.Build.VERSION.SDK_INT > MAX_API_FOR_MIX_RESOURCES) {
        ...
        try {
            ...
            return if (_root_ide_package_.android.os.Build.VERSION.SDK_INT > MAX_API_FOR_MIX_RESOURCES) {
                pluginResource
            } else {
                val hostResources = hostAppContext.resources
                MixResources(pluginResource, hostResources)
            }
        } catch (e: _root_ide_package_.android.content.pm.PackageManager.NameNotFoundException) {
     ...
     */
    private fun triggerWebViewHookResources(hostAppContext: _root_ide_package_.android.content.Context) {
        //先用宿主context初始化一个WebView，以便WebView的逻辑去修改sharedLibraryFiles，将webview.apk添加进去
        val latch = _root_ide_package_.java.util.concurrent.CountDownLatch(1)
        _root_ide_package_.android.os.Handler(_root_ide_package_.android.os.Looper.getMainLooper()).post {
            try {
                _root_ide_package_.android.webkit.WebView(hostAppContext)
    ...
    private fun fillApplicationInfoForNewerApi(
        applicationInfo: _root_ide_package_.android.content.pm.ApplicationInfo,
        hostApplicationInfo: _root_ide_package_.android.content.pm.ApplicationInfo,
